...
import org.kiwi.crius.tools.modle.ImportContext;
import org.kiwi.crius.tools.modle.NoopImportContext;
...
//	}
	...
	public String getJavaTypeName(Property p, boolean useGenerics, ImportContext importContext) {
		...
		if ( !StringHelper.isEmpty( overrideType ) ) {
            ...
            String rawType = null;
            try {
                rawType = getRawTypeName( p, useGenerics, true, importContext );
            } catch (Exception e) {
	...
	private String getRawTypeName(Property p, boolean useGenerics, boolean preferRawTypeNames, ImportContext importContext) throws Exception {
	...
	public static String getPreferredJavaType(String sqlType,int length,boolean nullable) throws UnsupportedDataTypeException{
		if(sqlType ==null || sqlType.equals("")) return null;
		String type =SqlDataType.forType(sqlType);
		if(type!=null) return type;
		if(sqlType.toLowerCase().equals("number")){
			if(length==1){
				...
				return nullable?Integer.class.getName():"int";
			}else if(length<19){
		...
		throw new UnsupportedDataTypeException("not supported.sqlType:"+sqlType);
	}
